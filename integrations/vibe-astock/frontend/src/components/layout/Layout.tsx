...
function Shell() {
  ...
  return <>
    ...
    <div className="flex h-dvh overflow-hidden">
      {!embedded && <aside aria-label="产品侧栏" className={cn('workspace-sidebar hidden shrink-0 flex-col md:flex', collapsed ? 'w-16' : 'w-64')}>{sidebar(collapsed)}</aside>}
      <div className="flex min-w-0 flex-1 flex-col overflow-hidden">
        {!embedded && <header className="workspace-topbar flex min-h-16 shrink-0 items-center justify-between gap-2 px-4 md:px-8">
    ...
    {!embedded && mobileOpen && <Dialog titleId="mobile-nav-title" close={() => setMobileOpen(false)} className="max-w-xs"><h2 id="mobile-nav-title" className="sr-only">产品导航</h2><div className="workspace-sidebar flex h-[85dvh] flex-col">{sidebar(false, true)}</div></Dialog>}
