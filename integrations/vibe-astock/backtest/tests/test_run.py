...
def test_missing_only_counts_symbols_the_user_asked_for():
    ...
    class FakeEngine:
        def __init__(self, cfg): self.trades = []
...
def test_runner_net_statistics_use_each_closed_trade_actual_fees(monkeypatch):
    ...
    class Engine:
        def __init__(self, cfg):
            self.trades = [Closed(110, 20), Closed(-100, 10)]
        def run_backtest(self, **kw):
            return {**BASE, 'profit_factor':1.1}
    monkeypatch.setattr(RUN, 'VibeLoader', Loader)
    monkeypatch.setattr(RUN, 'ENGINES', {**RUN.ENGINES, 'ChinaAEngine': Engine})
    result=RUN.run(PLAN, object())
    assert result.metrics['profit_factor']==1.1
    assert result.metrics['net_profit_factor']==pytest.approx(0.8182)
    assert result.metrics['net_profit_loss_ratio']==pytest.approx(0.8182)
    assert result.metrics['net_win_rate']==.5

@pytest.mark.parametrize('code,market,lot,fees', [('00700.HK','hk',100,True),('AAPL','us',123.45,False)])
def test_run_selects_actual_market_rules(monkeypatch,tmp_path,code,market,lot,fees):
    """真实 run 入口必须将港股规则传给引擎，不能静默使用美股默认。"""
    import backtest.run as module
    from backtest.engines.global_equity import GlobalEquityEngine
    seen=[]
    class Probe(GlobalEquityEngine):
        def run_backtest(self, **kwargs):
            seen.append((self.market,self.round_size(123.45,100),self.calc_commission(100,100,1,True)))
            raise RuntimeError('rule_probe_done')
    monkeypatch.setitem(module.ENGINES,'GlobalEquityEngine',Probe)
    plan=plan_backtest(codes=[code],start='2021-01-01',end='2025-12-31',style='long')
    with pytest.raises(RuntimeError,match='rule_probe_done'):
        module.run(plan,object(),run_dir=tmp_path)
    assert seen[0][0]==market
    assert seen[0][1]==lot
    assert (seen[0][2]>0)==fees
