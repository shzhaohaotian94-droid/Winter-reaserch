...
def test_frontend_uses_unified_agent_connection():
    """The embedded workbench should use the v1.1 unified, explicitly saved AI connection."""
    import pathlib

    llm = pathlib.Path("frontend/src/lib/llm.ts").read_text(encoding="utf-8")
    access = pathlib.Path("frontend/src/components/AgentAccess.tsx").read_text(encoding="utf-8")
    agent_api = pathlib.Path("frontend/src/lib/agent-api.ts").read_text(encoding="utf-8")
    deep_dive = pathlib.Path("frontend/src/components/ui/DeepDive.tsx").read_text(encoding="utf-8")
    assert "loadAgentConnection()" in llm
    assert "codex-private" in access
    assert "`/access/${kind}`" in access and "saveAgentConnection" in access
    assert 'window.dispatchEvent(new Event("astock-connection-changed"))' in agent_api
    assert "hasLlm()" in deep_dive
