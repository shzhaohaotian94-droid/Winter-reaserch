/** AStock adaptation of Research v1.1.0. Python owns and reaps the outer
 * POSIX group; children must NOT detach. Legacy credential-copy fallback is rejected. */
/**
 * 本机 Agent CLI 适配层。
 *
...
export async function probeClaude(env: NodeJS.ProcessEnv = process.env): Promise<LocalAgentStatus> {
  ...
  const probeTimeout = () => {
    ...
    if (remaining <= 0) throw new Error("Claude probe deadline exceeded");
    return Math.min(15_000, remaining);
  };
  let version: string | null = null;
  try {
    const versionCall = executableInvocation(bin, ["--version"], runEnv);
    const v = await execFileAsync(versionCall.file, versionCall.args, { env: runEnv, timeout: probeTimeout(), killSignal: 'SIGKILL', maxBuffer: 64 * 1024 });
    version = oneLine(v.stdout);
    const helpCall = executableInvocation(bin, ["--help"], runEnv);
    const h = await execFileAsync(helpCall.file, helpCall.args, { env: runEnv, timeout: probeTimeout(), killSignal: 'SIGKILL', maxBuffer: 128 * 1024 });
    ...
    return {
      provider: "cli-claude", name: "Claude Code", installed: true, authenticated: false,
      available: false, version, status: "probe_failed", detail: "Claude Code 已安装，但版本或功能检测未完成，请重试",
    };
  }
  try {
    const authCall = executableInvocation(bin, ["auth", "status"], runEnv);
    const a = await execFileAsync(authCall.file, authCall.args, { env: runEnv, timeout: probeTimeout(), killSignal: 'SIGKILL', maxBuffer: 64 * 1024 });
...
export async function runLocalAgent(agent: LocalAgentId, opts: RunLocalAgentOptions): Promise<string> {
  ...
  // Verify the selected billing source on every run, not just in the UI.
  if (agent === "claude") {
    const status = await probeClaude(baseEnv);
    if (!status.available) throw new LocalAgentError(
      status.status === "not_authenticated" ? "agent_not_authenticated" : "agent_probe_failed",
      status.status === "not_authenticated" ? "Claude 订阅未登录，请完成官方登录" : "Claude 状态检测未完成，请重试并检查版本或系统访问权限；现有登录未改变",
    );
  }
  ...
  return await new Promise<string>((resolve, reject) => {
    ...
    const child = spawn(launch.file, launch.args, {
      ...
      env: runEnv,
      stdio: ["pipe", "pipe", "pipe"],
      // Inherit Python's guarded process group; Python kills all descendants
      // after success/failure/cancel, and engine_guard also covers parent death.
      detached: false,
    });
    ...
    const cleanup = () => {
      ...
      // Workspace and slot are released in finally, including synchronous launch failures.
    };
    const finish = (fn: () => void) => {
      if (settled) return;
      settled = true;
    ...
    activeLocalAgents = Math.max(0, activeLocalAgents - 1);
    try { fs.rmSync(tmpDir, { recursive: true, force: true, maxRetries: 5, retryDelay: 100 }); }
    catch {
      if (primaryError instanceof Error) {
        primaryError.message += "；临时工作区清理失败，请检查本机文件权限";
      } else if (!primaryError) {
        throw new LocalAgentError("agent_cleanup_failed", `${label} 临时工作区清理失败，请检查本机文件权限`);
      }
    }
  });
}
