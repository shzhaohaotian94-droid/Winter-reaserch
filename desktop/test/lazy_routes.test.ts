...
test("保留全部路由，但非首屏业务按需加载并保留加载/错误反馈", () => {
  ...
  assert.deepEqual(paths, ["/", "/winter", "/research-library", "/daily-review", "/sentiment", "/astock", "/intel", "/intel/:tab", "/industry-news", "/signals", "/signals/:tab", "/sectors", "/sectors/:key", "/portfolio", "/stock-data", "/debate", "/backtest", "/watchlist", "/research", "/my-reports", "/notes", "/settings"]);
  assert.equal((source.match(/lazy: async/g) ?? []).length, 18);
