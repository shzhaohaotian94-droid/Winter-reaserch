...
export function WinterDashboard() {
    ...
    if (watchCodes.length) {
      api.quote(watchCodes.join(",")).then(setQuotes).catch(() => {
        ...
        watchCodes.forEach((code) => {
          const quote = publicQuoteSnapshot[code];
          if (quote) fallbackQuotes[code] = quote;
        });
  ...
  const marketLine = useMemo(() => {
    const a = indices.map((i) => `${i.name} ${i.change_pct == null ? "-" : `${i.change_pct > 0 ? "+" : ""}${i.change_pct}%`}`).join(" / ");
                ...
                <div key={c} className="group flex items-center justify-between gap-3 rounded-lg bg-muted/25 px-3 py-2">
                  ...
                  <div className="flex items-center gap-2">
                    <div className="text-right">
                      ...
                      <p className={cn("text-xs", q ? pctColor(q.change_pct) : "text-muted-foreground")}>{q?.change_pct == null ? "-" : `${q.change_pct > 0 ? "+" : ""}${q.change_pct}%`}</p>
                    </div>
